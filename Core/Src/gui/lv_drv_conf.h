...
#define DISP_HOR_RES DISPLAY_W
#define DISP_VER_RES DISPLAY_H
...
#define LV_DRV_DISP_CMD_DATA(val)  DISPLAY_DC(val) /*pin_x_set(val)*/    /*Set the command/data pin to 'val'*/
#define LV_DRV_DISP_RST(val)       DISPLAY_RESET(val) /*pin_x_set(val)*/    /*Set the reset pin to 'val'*/

/*---------
 *  SPI
 *---------*/
#define LV_DRV_DISP_SPI_CS(val)          DISPLAY_CS(val) /*spi_cs_set(val)*/     /*Set the SPI's Chip select to 'val'*/
#define LV_DRV_DISP_SPI_WR_BYTE(data)    do{ LV_DRV_DISP_SPI_CS(0); DISPLAY_WR_BYTE(data); LV_DRV_DISP_SPI_CS(1); }while(0) /*spi_wr(data)*/        /*Write a byte the SPI bus*/
#define LV_DRV_DISP_SPI_WR_ARRAY(adr, n) do{ LV_DRV_DISP_SPI_CS(0); DISPLAY_WR_ARRAY(adr,n); LV_DRV_DISP_SPI_CS(1); }while(0) /*spi_wr_mem(adr, n)*/  /*Write 'n' bytes to SPI bus from 'adr'*/
